...
export function resizeBounds(bounds: XYWH, corner: Side, point: Point): XYWH {
  ...
  if ((corner & Side.Left) === Side.Left) {
    ...
    result.width = Math.abs(bounds.x + bounds.width - result.x);
  }
  ...
  if ((corner & Side.Bottom) === Side.Bottom) {
    console.log(corner, Side.Bottom);
...
export function petPointToPathLayer(points: number[][], color: Color): PathLayer {
  if (points.length < 2) {
